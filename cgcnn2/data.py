...
def collate_pool(dataset_list) -> tuple[tuple[torch.Tensor, torch.Tensor, torch.Tensor], torch.Tensor, list[str | int]]:
    """
...
class CIFData(Dataset):
    ...
    def _load_item_fast(self, idx) -> tuple[tuple[torch.Tensor, torch.Tensor, torch.Tensor], torch.Tensor, str | int]:
        cif_id, target = self.id_prop_data[idx]
        crystal = Structure.from_file(os.path.join(self.root_dir, cif_id + ".cif"))
        atom_fea = np.vstack(
            [
...
class CIFData_NoTarget(Dataset):
    ...
    def __getitem__(self, idx) -> tuple[tuple[torch.Tensor, torch.Tensor, torch.Tensor], torch.Tensor, str | int]:
        cif_id, target = self.id_prop_data[idx]
        crystal = Structure.from_file(os.path.join(self.root_dir, cif_id + ".cif"))
        atom_fea = np.vstack(
            [
